...
// add
#include "third_party/blink/renderer/modules/canvas/canvas2d/base_rendering_context_2d_random.h"
...
void BaseRenderingContext2D::fillRect(double x,
  ...
  // add
  float fx = ClampTo<float>(x) + get_random_num();
  float fy = ClampTo<float>(y) + get_random_num();
  float fwidth = ClampTo<float>(width) + get_random_num();
  float fheight = ClampTo<float>(height) + get_random_num();
